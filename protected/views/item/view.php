...
$this->breadcrumbs[$model->name] = array("/item/{$model->entry}");
?>
    ...
    <?php if($model->class == $model::ITEM_CLASS_WEAPON || ($model->class == $model::ITEM_CLASS_ARMOR && !in_array($model->InventoryType, array(2, 11, 12, 28)))): ?>
        <div class="snippet">
            <div class="model" id="model-<?=$model->entry?>">
                <div class="viewer" style="background-image: url(http://media.blizzard.com/wow/renders/items/item<?=$model->entry?>.jpg);"></div>
            </div>
