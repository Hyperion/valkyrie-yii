...
class ItemTemplate extends Base\World
{
    ...
    public function search()
    {
        ...
        if($this->minlvl)
            $criteria->compare('ItemLevel', '>=' . $this->minlvl);
        if($this->maxlvl)
            $criteria->compare('ItemLevel', '<=' . $this->maxlvl);

        if($this->minrlvl)
            $criteria->compare('RequiredLevel', '>=' . $this->minrlvl);
        if($this->maxrlvl)
            $criteria->compare('RequiredLevel', '<=' . $this->maxrlvl);
        ...
        if($dropCreatures === false)
        {
            ...
            if($drops_cr)
            {
                $rows = $this->dbConnection->createCommand()
                    ->select('entry, name, type, minlevel, maxlevel, lootid')
                    ->from('creature_template')
                    ->where(array('in', 'lootid', array_keys($drops_cr)))
                    ->queryAll();

                foreach($rows as $row)
                    $dropCreatures[] = array_merge($row, $drops_cr[$row['lootid']]);
            }
