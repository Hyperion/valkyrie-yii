<span  class="icon-frame frame-56" style='background-image: url("http://eu.battle.net/wow-assets/static/images/icons/56/<?php echo $model->icon ?>.jpg");'>
    <?php if($model->stackable > 1 && !is_array($data)): ?>
        <span class="stack"><?php echo $model->stackable ?></span>
    <?php endif; ?>
</span>
<?php if(is_array($data)): ?>
    <h3 class="subheader color-q<?php echo $model->Quality ?>"><?php echo $model->name ?></h3>
<?php endif; ?>
...
if($model->class == $model::ITEM_CLASS_WEAPON && $model->delay):
    ?>
        <li>
            <span class="float-right">Speed <?php echo $model->delay / 1000 ?></span>
...
if($model->RequiredLevel):
    ...
    foreach($model->spells as $spell):
        ?>
        <li class="color-q2">
            <a class = "has-tip" href="/wow/spell/<?php echo $spell['spellid'] ?>">
...
if($model->itemset):
    ...
    <?php foreach($model->set['items'] as $item): ?>
                    <li class="indent">
                        <a class="color-<?php echo ((in_array($item['entry'], $equipped)) ? 'tooltip-beige' : 'd4') ?> has-tip" href="/wow/item/<?php echo $item['entry'] ?>"><?php echo $item['name'] ?></a>
                    </li>
    <?php endforeach; ?>
                <li class="indent-top"></li>
    <?php foreach($model->set['bonuses'] as $piece => $spell): ?>
                    <li class="color-<?php echo (($piece <= $count) ? 'tooltip-green' : 'd4') ?>">(<?php echo $piece ?>) Set: <a class = "has-tip" href="/wow/spell/<?php echo $spell->spellID ?>"><?php echo $spell->info ?></a></li>
    <?php endforeach; ?>
            </ul>
        </li>
<?php endif; ?>
