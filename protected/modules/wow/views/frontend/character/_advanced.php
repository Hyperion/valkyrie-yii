...
<?php foreach($model->items as $slot => $item) :
    if(!isset($item['entry'])): ?>
<div data-id="<?=($item['slot']-1)?>" data-type="<?=$item['slot']?>" class="slot <?=(($slot > 4 && $slot < 16 && $slot !=8 && $slot != 14) ? 'slot-align-right' : null)?> slot-<?=$item['slot']?>" id="slot-<?=$slot?>">
...
<?php continue; endif; ?>
<div data-id="<?=($item['slot']-1)?>" data-type="<?=$item['slot']?>" class="slot slot-<?=$item['slot']?> <?=(($slot > 4 && $slot < 16 && $slot !=8 && $slot != 14) ? 'slot-align-right' : null)?> item-quality-<?=$item['quality']?>" id="slot-<?=$slot?>">
	<div class="slot-inner">
    	<div class="slot-contents">
            ...
            <div class="details">
                ...
                <span class="name color-q<?=$item['quality']?>">
                    <?=((($slot > 4 && $slot < 16 && $slot !=8 && $slot != 14) && !$item['enchant_id'] && $item['can_enchanted']) ? '<a href="javascript:;" class="audit-warning"></a>' : null)?>
                    <a href="/wow/item/<?=$item['entry']?>" data-item="<?=$item['data']?>"><?=$item['name']?></a>
                    <?=((($slot < 5 || $slot > 15 || $slot == 8 || $slot == 14) && !$item['enchant_id'] && $item['can_enchanted']) ? '<a href="javascript:;" class="audit-warning"></a>' : null)?>
                </span>
